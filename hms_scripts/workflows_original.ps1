...
# Remote app should be published as powershell.exe and require argument c:\users\public\documents\workflows_original.ps1
...
if ($env:userdomain.tolower() -like "iqms-cloud") {
    $binding = "$($custPrefix).workflows.$custdomain"
} else {
    $binding = "workflows.$custdomain"
}
