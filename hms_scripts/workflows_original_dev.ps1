...
# Remote app should be published as powershell.exe and require argument c:\users\public\documents\workflows_dev.ps1
...
if ($env:userdomain.tolower() -like "iqms-cloud") {
    $binding = "$($custPrefix)dev.workflows.$custdomain"
} else {
    $binding = "workflowsdev.$custdomain"
}
