...
# Remote app should be published as powershell.exe and require argument c:\users\public\documents\workflows_iqdev.ps1
...
if ($env:userdomain.tolower() -like "iqms-cloud") {
    $binding = "$($custPrefix).iqdev.$custdomain"
} else {
    $binding = "iqdev.$custdomain"
}
