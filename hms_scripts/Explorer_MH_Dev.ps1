...
if ($custDomain -eq 'iqms-cloud.net') {
    Start-Process -FilePath 'c:\windows\sysWow64\explorer.exe' -ArgumentList "\\$($custPrefix).dev.iqms-cloud.net\IQMS";
} else {
    Start-Process -FilePath 'c:\windows\sysWow64\explorer.exe' -ArgumentList "\\dev.$($custPrefix)-cloud.net\IQMS";
}   
