...
# Remote app should be published as powershell.exe and require argument c:\users\public\documents\AssyData.ps1
...
if (Test-Path -Path $printerMap) {
    Start-Process -FilePath $printerMap
}

# launch applications
Write-Host -ForegroundColor yellow "`nLaunching application..."
Set-Location "c:\Program Files (x86)\IQMS\IQWin32\"
Start-Process -FilePath "c:\Program Files (x86)\IQMS\IQWin32\AssyData.exe" -Verbose
