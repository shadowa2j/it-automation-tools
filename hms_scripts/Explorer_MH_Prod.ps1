...
# Remote app should be published as powershell.exe and require argument c:\users\public\documents\FileExplorer_MapDrive.ps1
...
Stop-Process -processname explorer -ErrorAction ignore
Start-Sleep 1

if ($custDomain -eq 'iqms-cloud.net') {
    Start-Process -FilePath 'c:\windows\sysWow64\explorer.exe' -ArgumentList "\\$($custPrefix).alert.iqms-cloud.net\IQMS";
} else {
    Start-Process -FilePath 'c:\windows\sysWow64\explorer.exe' -ArgumentList "\\alert.$($custPrefix)-cloud.net\IQMS";
}   
