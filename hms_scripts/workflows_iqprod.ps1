...
# Remote app should be published as powershell.exe and require argument c:\users\public\documents\workflows_iqprod.ps1
...
if ($env:userdomain.tolower() -like "iqms-cloud") {
    $binding = "$($custPrefix).iqprod.$custdomain"
} else {
    $binding = "iqprod.$custdomain"
}
